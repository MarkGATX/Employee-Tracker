...
async function deleteEmp() {
    ...
    inquirer.prompt([
        ...
        {
            type: 'list',
            message: "Are you sure? There is no recovering a deleted employee.",
            name: 'delVerify',
        ...
        .then((response) => {
            if (response.delVerify !== 'Yes, delete it.') {
                console.log("Second thoughts accepted. Employee will continue to feed family.")
                index.selectAction();
                return;
            } else {
                empDelId = '';
                for (i = 0; i < empIds.length; i++) {
                    if (empIds[i].name === response.delEmpName) {
                        empDelId = empIds[i].id;
                        break;
                    }
                }
                db.promise().query('DELETE FROM employee WHERE employee.id = ?', empDelId, (err, result) => {
                    if (err) {
                        console.log(err);
                    }
                    ...
                    .then(() => {
                        console.log(`${response.delEmpName} was permanently deleted.`);
                        index.selectAction();});
            }
        })
...
module.exports = { viewEmpRoles, viewAllEmp, addEmp, updateEmpRole, updateEmpMgr, viewEmpByMgr, deleteEmp }