...
async function viewTotalBudg() {
    ...
    inquirer.prompt([
        ...
        .then((response) => {
            ...
            db.promise().query('SELECT department.deptName AS department, SUM(empRole.salary) AS budget FROM empRole JOIN department ON department.id = emprole.department_id WHERE emprole.department_id = ?', deptSortId, (err, result) => {
                if (err) {
                    console.log(err);
                }
                console.log(result);
...
module.exports = { viewDept, addDept, viewEmpByDept, viewTotalBudg }
